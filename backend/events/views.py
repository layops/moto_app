...
class EventViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if group_pk:
            ...
            # Tüm etkinlikler (kullanıcının grupları veya kişisel/public)
            user_groups = Group.objects.filter(members=user) | Group.objects.filter(owner=user)
            return Event.objects.filter(
                Q(group__in=user_groups) | Q(group__isnull=True, organizer=user) | Q(is_public=True)
    ...
    def perform_update(self, serializer):
        ...
        if event.organizer != user and (event.group and event.group.owner != user):
            raise PermissionDenied("Bu etkinliği düzenleme yetkiniz yok.")
        serializer.save()

    def perform_destroy(self, instance):
        user = self.request.user
        if instance.organizer != user and (instance.group and instance.group.owner != user):
    ...
    def join(self, request, pk=None):
        ...
        if event.is_full():
            return Response({"error": "Etkinlik kontenjanı dolmuştur."}, status=status.HTTP_400_BAD_REQUEST)
        if user in event.participants.all():
            return Response({"error": "Zaten bu etkinliğe katılıyorsunuz."}, status=status.HTTP_400_BAD_REQUEST)

        event.participants.add(user)
        return Response({"message": "Etkinliğe başarıyla katıldınız."}, status=status.HTTP_200_OK)
    ...
    def leave(self, request, pk=None):
        ...
        if user not in event.participants.all():
            return Response({"error": "Bu etkinliğe zaten katılmıyorsunuz."}, status=status.HTTP_400_BAD_REQUEST)

        event.participants.remove(user)
        return Response({"message": "Etkinlikten ayrıldınız."}, status=status.HTTP_200_OK)
    ...
    def get_queryset(self):
        ...
        if group_pk:
            ...
            # Tüm etkinlikler: kullanıcıya açık, kullanıcıya ait veya grubuna ait
